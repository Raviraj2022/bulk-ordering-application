...
import EditCorporateMealPlan from '../../Forms/EditCorporateMealPlan';
...
const CorporateMealPlanDataTable: React.FC<EditFormProps> = ({
  handleEditForm,
}) => {
  ...
  const fetchDishes = async (role: string) => {
    try {
      let response;
      if (role === 'admin') {
        response = await axios.get(`${baseUrl}/api/corporatemealplan`);
      } else if (role === 'company_admin') {
        const companyId = localStorage.getItem('companyID');
        response = await axios.get(
          `${baseUrl}/api/corporatemealplan/company/${companyId}`,
        );
      } else if (role === 'branch_manager') {
        const branchId = localStorage.getItem('branchId');
        response = await axios.get(
          `${baseUrl}/api/corporatemealplan/branch/${branchId}`,
        );
      }
  ...
  const fetchDishData = async () => {
    try {
      const response = await axios.get(`${baseUrl}/api/corporatemealplan`);
      const data = response.data;
      // console.log('Dish data:', data[0].corporate_company_id);
      const DishObj = {};
      data.forEach((dish: any) => {
        // console.log(dish.corporate_manager_id);
        DishObj[dish.corporate_meal_plan_id] = dish.corporate_meal_plan_name;
      });
      //   console.log(DishObj);
      setDishData(DishObj);
    } catch (error) {
  ...
  const currentDishes = Array.isArray(dishes)
    ? dishes.slice(indexOfFirstItem, indexOfLastItem)
    : [];
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      await axios.delete(`${baseUrl}/api/corporatemealplan/${id}`);
      fetchDishes(userRole);
      fetchDishData();
      setDeletePopup(true);
    } catch (error) {
                ...
                {currentDishes.map((packageItem, key) => (
                  ...
                  >
                    <td className="border-[#eee] py-5 px-4 pt-6 dark:border-strokedark flex items-center">
                      <img
                        src={packageItem.corporate_MealPlanImgUrl}
                        className="h-12 w-12 mr-2 rounded-full"
                        alt="User"
                      />
                      <span>{packageItem.corporate_meal_plan_name}</span>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
                      {parseFloat(
                        packageItem.corporate_daily_breakfast_price,
                      ).toFixed(2)}
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      {packageItem.dietary_choices.map((dietary, index) => (
                        <span
                          key={index}
                          className="inline-flex rounded-full bg-opacity-10 py-1 px-3 mt-1 text-sm font-medium bg-primary text-primary"
                        >
                          {dietary.name}
                        </span>
                      ))}
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p
                        className={`inline-flex rounded-full capitalize bg-opacity-10 py-1 px-3 text-sm font-medium ${
                          packageItem.corporate_meal_plan_availability_status ===
                          'available'
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button
                          ...
                          onClick={() => {
                            Setshow(true);
                            SetID(packageItem.corporate_meal_plan_id);
                            handleEditFormChild();
                          }}
                        >
                        ...
                        {isAdmin && (
                          <button
                            className="hover:text-primary"
                            onClick={() =>
                              handleDelete(packageItem.corporate_meal_plan_id)
                            }
                          >
                            <svg
          ...
          {showPaginator && (
            <nav aria-label="Page navigation example">
              <ul className="flex items-center -space-x-px h-8 text-sm">
                {Array.from({
                  length: Math.ceil(dishes.length / itemsPerPage),
                }).map((_, index) => (
                  <li key={index}>
                    <button
...
export default CorporateMealPlanDataTable;
