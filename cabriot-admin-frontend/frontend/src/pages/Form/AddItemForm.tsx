...
const DishForm = () => {
    ...
    const fetchData = async () => {
      try {
        const dietaryChoicesResponse = await axios.get(
          'http://127.0.0.1:7000/api/dietary',
        );
        const cuisineChoicesResponse = await axios.get(
          'http://127.0.0.1:7000/api/cusisine',
        );
        const mealChoicesResponse = await axios.get(
          'http://127.0.0.1:7000/api/mealchioce',
        );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://127.0.0.1:7000/api/dishinfo`, formData);
    ...
    <DefaultLayout>
      ...
      industry. */}
          ...
          <div className="flex justify-end text-lg cursor-auto p-2">
            <h1 onClick={() => setForm(true)} className="cursor-pointer">
              Back
            </h1>
          </div>
                      ...
                      <div className="">
                        ...
                        <label className="switch">
                          <input
                            type="checkbox"
                            checked={
                              formData.dish_availability_status === 'available'
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                dish_availability_status: e.target.checked
                                  ? 'available'
                                  : 'unavailable',
                              })
                            }
                      ...
                      <div>
                        <div className="">
                          ...
                          {formData.cuisine_choices.map((cuisine, index) => (
                            <span
                              key={index}
                              className=" border-2 inline-flex items-center px-3 py-1 rounded-lg bg-gray-200 text-lg text-gray-800 dark:text-gray-200"
                            >
                              {cuisine == 1 ? 'North Indian' : 'South Indian'}
                              <FaTimes
                                onClick={() =>
                                  handleRemove(index, 'cuisine_choices')
                                }
                      ...
                      <div className="">
                        ...
                        <div>
                          {formData.meal_choices.map((meal, index) => (
                            <span
                              key={index}
                              className="border-2 inline-flex items-center px-3 py-1 rounded-lg text-lg text-gray-800 dark:text-gray-200"
                            >
                              {meal}
                              <FaTimes
                                onClick={() =>
                                  handleRemove(index, 'meal_choices')
                                }
