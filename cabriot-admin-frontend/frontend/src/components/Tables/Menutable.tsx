...
const MenuTable = ({ ID }: Menuid) => {
    ...
    const fetchMenus = async () => {
      try {
        const response = await axios.get(
          `http://localhost:7000/api/menu/${ID} `,
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:7000/api/menu/${id}`);
        ...
        <div className="flex flex-col">
          <div className="grid grid-cols-7 rounded-sm dark:bg-meta-4">
            <div className="p-1 text-center">
              <h5 className="font-semibold">Item Image</h5>
            </div>
            <div className="p-1 text-center">
            ...
            <div className="p-1 text-center">
              <h5 className="font-semibold">Calories</h5>
            </div>
            <div className="p-1 text-center">
