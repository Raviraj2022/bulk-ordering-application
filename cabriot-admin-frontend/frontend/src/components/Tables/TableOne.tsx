...
const TableOne = ({ ID }: tabelId) => {
    ...
    const fetchKitchens = async () => {
      try {
        const response = await axios.get(
          `http://localhost:7000/api/kitchen/${ID}`,
        );
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        const menuResponse = await axios.get(`http://localhost:7000/api/menu/`);
    ...
    const fetchMenuItem = async () => {
      try {
        ...
        const MenuResponse = await axios.get(
          `http://localhost:7000/api/menu/${selectedMenu.id}`,
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:7000/api/kitchen/${id}`);
