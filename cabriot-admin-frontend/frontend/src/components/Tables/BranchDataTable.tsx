...
const BranchDataTable: React.FC<EditFormProps> = ({ handleEditForm }) => {
  const [branches, setBranches] = useState([]);
  const [show, setShow] = useState(false);
  const [ID, setID] = useState('');
  const [showBranchDeletePopup, setBranchDeleteShowPopup] = useState(false);
  ...
  const fetchBranches = async () => {
    try {
      ...
      if (userRole === 'admin') {
        console.log('super');
        response = await axios.get(`${baseUrl}/api/branch-data/`);
      } else {
        console.log('user');
  ...
  const handleDelete = async (id) => {
    try {
      ...
      //   );
      await axios.delete(`${baseUrl}/api/branch-data/${id}/`);
      const updatedBranches = branches.filter((branch) => branch.id !== id);
      setBranches(updatedBranches);
      setBranchDeleteShowPopup(true);
    } catch (error) {
      ...
      {showBranchDeletePopup && (
        <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-gray-800 bg-opacity-50">
          <div className="bg-white p-8 rounded shadow-md">
            ...
            <button
              onClick={() => {
                setBranchDeleteShowPopup(false);
              }}
