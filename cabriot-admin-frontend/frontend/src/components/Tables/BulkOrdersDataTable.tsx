...
const BulkOrdersDataTable: React.FC<EditFormProps> = ({handleEditForm}) => {
    ...
    const fetchDishes = async () => {
        try {
            ...
            const response = await axios.get(`${baseUrl}/api/bulk-order`);
    ...
    const currentOrders = bulkOrder.slice(indexOfFirstItem, indexOfLastItem);
    console.log('Current currentOrders:', currentOrders);
                ...
                <div className="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
                    ...
                    <div className="w-full">
                        <h5 className="mb-3 text-lg font-semibold text-black dark:text-[#34D399] ">
                            Specific Bulk Order Deleted Successfully
                        </h5>
                    </div>
                </div>
            )}
            {!show && <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="max-w-full overflow-x-auto">
                    <table className="w-full table-auto">
                        ...
                        <tbody>{currentOrders.map((packageItem, key) => (
                            ...
                            <tr className="border-b border-[#eee] dark:border-strokedark" key={key}>
                                <td className="border-[#eee] py-5 px-4 pt-6 dark:border-strokedark flex items-center">
                                    <span>{packageItem.company.company_name}</span>
                                </td>
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
                                    {packageItem.MealPlan && packageItem.MealPlan.length > 0 ? (
                                        packageItem.MealPlan.map((item, index) => (
                                            <span key={index}>{item.meal_plan_name}</span>
                                        ))
                                    ) : (
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
                                    {packageItem.kitchen && packageItem.kitchen.length > 0 ? (
                                        packageItem.kitchen.map((item, index) => (
                                            <span key={index}>{item.kitchenName}</span>
                                        ))
                                    ) : (
                                        <span>No kitchen available</span>
                                    )}
                                </td>
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <span>{packageItem.per_day_meal_qty}</span>
                                </td>

                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <span>{packageItem.bulk_order_start_date}</span>
                                </td>
                                
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <span>{packageItem.bulk_order_end_date}</span>
                                </td>
...
export default BulkOrdersDataTable;
