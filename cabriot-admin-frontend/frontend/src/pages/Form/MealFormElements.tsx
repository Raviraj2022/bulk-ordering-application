...
const MealFormElements = () => {
  ...
  const [quantities, setQuantities] = useState(
    Array(addedDishes.length).fill(0),
  );
  ...
  const [dishesMealsformData, setDishesMealsFormData] = useState<
    DishesMealsFormData[]
  >([
    {
      id: 0,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const finalList = formData.selected_dishes.map((dish, index) => [
      dish,
      formData.meal_qty[index],
    ]);
                      ...
                      <div className="">
                        ...
                        <label className="switch">
                          <input
                            type="checkbox"
                            checked={
                              formData.meal_availability_status === 'available'
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                meal_availability_status: e.target.checked
                                  ? 'available'
                                  : 'unavailable',
                              })
                            }
