...
const MenuFormElements = () => {
  ...
  const submitNewMenu = async (e: FormEvent<HTMLFormElement>) => {
    if (!isFormReady) {
      ...
      e.preventDefault();
      try {
        const filteredFormData = dishesMealsformData.filter(
          (form) => form.id !== 0,
        );
        ...
        const createKitchenMenus = await axios.post(
          `${baseUrl}/api/menu/kitchenmenu`,
          formDataToSend,
        );
        setFormData({
          kitchens: [],
  ...
  useEffect(() => {
    ...
    const fetchDishes = async () => {
      try {
        ...
        const allDishesData = dishesData.map((dish) => ({
          id: dish.dish_id,
          name: dish.dish_name,
        }));
        const allMealsData = mealsData.map((meal) => ({
          id: meal.meal_id,
          name: meal.meal_name,
        }));
